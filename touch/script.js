...
window.addEventListener("touchstart", (e) => {
	if (Math.sqrt(Math.pow(flick.centre.x - e.changedTouches[0].clientX, 2) + Math.pow(flick.centre.y - e.changedTouches[0].clientY, 2)) < flick.ballRadius && flick.refresh === 0) {
		flick.moving = true;
		flick.movements.push([e.changedTouches[0].clientX, e.changedTouches[0].clientY]);
	}
});
//On mouse move
window.addEventListener("touchmove", (e) => {
	...
	console.log(e);
	if (flick.moving) {
		if (Math.sqrt(Math.pow(flick.centre.x - e.changedTouches[0].clientX, 2) + Math.pow(flick.centre.y - e.changedTouches[0].clientY, 2)) < flick.ballRadius) {
			flick.movements.push([e.changedTouches[0].clientX, e.changedTouches[0].clientY]);
		} else {
			shoot(e);
		}
...
let shoot = (e) => {
	if (flick.moving) {
		...
		let vx, vy;
		do {
			vx = e.changedTouches[0].clientX - flick.movements[flick.movements.length - i][0];
			vy = e.changedTouches[0].clientY - flick.movements[flick.movements.length - i][1];
			i++;
		} while (vx === 0 && vy === 0 && i <= flick.movements.length);
		flick.balls.push(new Ball(e.changedTouches[0].clientX, e.changedTouches[0].clientY, vx, vy, flick.colour));
